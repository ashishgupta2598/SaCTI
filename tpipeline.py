...
class TPipeline:
    ...
    def _prepare_posdep(self):
        ...
        self._config.itos[DEPRELG] = {v: k for k, v in self.train_set.vocabs[DEPRELG].items()}
    ...
    def _eval_posdep(self, data_set, batch_num, name, epoch,task='dev'):
        torch.cuda.empty_cache()
        self._embedding_layers.eval()
        self._tagger.eval()
        ...
        for batch in DataLoader(data_set, batch_size=self._config.batch_size,
            ...
            sentlens = [l + 1 for l in batch.word_num]
            head_seqsg = [chuliu_edmonds_one_root(adj[:l, :l])[1:] for adj, l in
                         zip(predicted_depg[0], sentlens)]
            deprel_seqsg = [[self._config.itos[DEPRELG][predicted_depg[1][i][j + 1][h]] for j, h in
                            enumerate(hs)] for
                           i, hs
                           in
                           enumerate(head_seqsg)]
            ...
            for bid in range(batch_size):
                ...
                #print(words)
                words_matrix_small.append(words)

                # print(len(words), len(head_seqs[bid]))
