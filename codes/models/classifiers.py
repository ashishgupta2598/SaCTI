...
from layers.crf_layer import CRFLoss, viterbi_decode
...
class PosDepClassifier(nn.Module):
    def __init__(self, config,panelty, treebank_name):
        super().__init__()
        self.config = config
        ...
        self.unlabeled_grammer = Deep_Biaffine(self.down_dim, self.down_dim,
                                       self.down_dim, 1)
        self.deprel_grammer = Deep_Biaffine(self.down_dim, self.down_dim,
                                    self.down_dim, len(self.vocabs[DEPRELG]))
    ...
    def forward(self, batch, word_reprs, cls_reprs):
        ...
        # print("nromal heads ",batch.head_idxs)#Correct
        diag = torch.eye(batch.head_idxs.size(-1) + 1, dtype=torch.bool).cuda().unsqueeze(0)
        #print("yolo ",diag.shape,unlabeled_scores.shape)
        ...
        deprel_scores = torch.gather(deprel_scores, 2,
            ...
            #understand this gather waali line...and learn one more time that thing with u
        #print("shape dep real socreess ",deprel_scores.shape)
        ...
        ###GRAMMER HEAD
        unlabeled_scores_grammer = self.unlabeled_grammer(dep_reprs, dep_reprs).squeeze(3)
        diag = torch.eye(batch.headg_idxs.size(-1) + 1, dtype=torch.bool).cuda().unsqueeze(0)
        unlabeled_scores_grammer.masked_fill_(diag, -float('inf'))
        unlabeled_scores_grammer = unlabeled_scores_grammer[:, 1:, :]  
        unlabeled_scores_grammer = unlabeled_scores_grammer.masked_fill(batch.word_mask.unsqueeze(1), -float('inf'))
        unlabeled_target_grammer = batch.headg_idxs.masked_fill(batch.word_mask[:, 1:], -100)
        loss += self.panelty*(self.criteria(unlabeled_scores_grammer.contiguous().view(-1, unlabeled_scores_grammer.size(2)),
                              unlabeled_target_grammer.view(-1)))
        
        deprel_scores_grammer = self.deprel_grammer(dep_reprs, dep_reprs)
        deprel_scores_grammer = deprel_scores_grammer[:, 1:]  
        deprel_scores_grammer = torch.gather(deprel_scores_grammer, 2,
                                     batch.headg_idxs.unsqueeze(2).unsqueeze(3).expand(-1, -1, -1, len(
                                         self.vocabs[DEPRELG]))).view(
            -1, len(self.vocabs[DEPRELG]))
        deprel_target_grammer = batch.deprelg_idxs.masked_fill(batch.word_mask[:, 1:], -100)
        loss += self.panelty*(self.criteria(deprel_scores_grammer.contiguous(), deprel_target_grammer.view(-1)))
        # print(loss)
        return loss
    ...
    def predict(self, batch, word_reprs, cls_reprs):
        ...
        unlabeled_scores.masked_fill_(diag, -float('inf'))
        #print("hello ",unlabeled_scores.shape)

        # deprel
        deprel_scores = self.deprel(dep_reprs, dep_reprs)
        #store_deprel_scores.append(deprel_scores)
        #print("these are such as ",deprel_scores.shape)
        dep_preds = []
        dep_preds.append(F.log_softmax(unlabeled_scores, 2).detach().cpu().numpy())
        #print("dep before ",deprel_scores.max(3).shape)
        dep_preds.append(deprel_scores.max(3)[1].detach().cpu().numpy())
        #print("dep preds are as such ",dep_preds.shape)
        # print(dep_preds)
        # exit()
        ###Here is the keyGRAMMER
        unlabeled_scores_grammer = self.unlabeled_grammer(dep_reprs, dep_reprs).squeeze(3)

        diag1 = torch.eye(batch.headg_idxs.size(-1) + 1, dtype=torch.bool).unsqueeze(0).to(self.config.device)
        unlabeled_scores_grammer.masked_fill_(diag1, -float('inf'))

        # deprel
        deprel_scores_grammer = self.deprel_grammer(dep_reprs, dep_reprs)
        dep_preds_grammer = []
        dep_preds_grammer.append(F.log_softmax(unlabeled_scores_grammer, 2).detach().cpu().numpy())
        dep_preds_grammer.append(deprel_scores_grammer.max(3)[1].detach().cpu().numpy())
        #print(unlabeled_scores_grammer)
